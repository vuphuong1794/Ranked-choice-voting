...
export default (
  ...
  //key is nominationID, value is score
  //initialize scores object to store scores for each nomination
  const scores: { [nominationID: string]: number } = {};

  //loop through each user's rankings
  Object.values(rankings).forEach((userRankings) => {
    userRankings.forEach((nominationID, n) => {
      ...
      //accumulate score for each nominationID
